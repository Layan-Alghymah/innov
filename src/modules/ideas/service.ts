...
export type IdeaErrorCode = "VALIDATION" | "NOT_DRAFT" | "INVALID_TRANSITION" | "NOT_AUTHOR" | "BAD_ACTIVITY";
...
/** List ideas visible to the caller (scope-filtered server-side), optional status filter. */
export async function listIdeasInScope(actor: AccessContext, opts?: { status?: IdeaStatus }): Promise<IdeaListRow[]> {
  requirePermission(actor, IDEA_VIEW);
  const scope = ideaScopeWhere(actor);
  const rows = await prisma.idea.findMany({
    where: opts?.status ? { AND: [scope, { status: opts.status }] } : scope,
